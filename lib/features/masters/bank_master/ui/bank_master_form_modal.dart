...
/// Add / edit bank in [AppFormModal].
class BankMasterFormModal {
  static Future<void> show(
    BuildContext context, {
    BankMasterModel? existing,
  }) {
    final p = context.read<BankMasterProvider>();
...
class _BankMasterFormDialog extends StatefulWidget {
  const _BankMasterFormDialog({this.existing});

  final BankMasterModel? existing;

  @override
  State<_BankMasterFormDialog> createState() => _BankMasterFormDialogState();
}

class _BankMasterFormDialogState extends State<_BankMasterFormDialog> {
  ...
  Future<void> _save(BuildContext dialogContext) async {
    ...
    final p = dialogContext.read<BankMasterProvider>();
    ...
    messenger.showSnackBar(
      SnackBar(
        content: Text(
          _isEdit ? 'Bank updated' : 'Bank created',
          style: GoogleFonts.poppins(
            fontSize: AppTokens.bodySize,
            color: AppTokens.white,
          ),
  ...
  Widget build(BuildContext context) {
    ...
    final title = _isEdit ? 'Edit Bank Master' : 'Add Bank Master';
