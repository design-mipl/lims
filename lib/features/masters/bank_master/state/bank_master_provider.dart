...
import '../data/bank_master_api.dart';
import '../models/bank_master_model.dart';

class BankMasterProvider extends BaseProvider {
  BankMasterProvider({BankMasterApi? api}) : _api = api ?? sl<BankMasterApi>();

  final BankMasterApi _api;

  List<BankMasterModel> _items = [];
  String _searchQuery = '';
  MasterStatus? _selectedStatusFilter;
  int _currentPage = 1;
  int _pageSize = 10;

  List<BankMasterModel> get items => List<BankMasterModel>.unmodifiable(_items);
  ...
  List<BankMasterModel> get filteredItems {
    final q = _searchQuery.trim().toLowerCase();
    Iterable<BankMasterModel> list = _items;
  ...
  List<BankMasterModel> get pagedRows {
  ...
  Future<void> create({
    ...
    await runAsync(() async {
      await _api.create(code: code, branch: branch, status: status);
      _items = await _api.fetchAll();
      _clampCurrentPage();
    });
  }
