...
import 'problem_master_form_modal.dart';

class ProblemMasterScreen extends StatefulWidget {
  const ProblemMasterScreen({super.key});

  @override
  State<ProblemMasterScreen> createState() => _ProblemMasterScreenState();
}

class _ProblemMasterScreenState extends State<ProblemMasterScreen> {
  ProblemMasterProvider? _provider;
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (!mounted) return;
      _provider = context.read<ProblemMasterProvider>();
  ...
  Future<void> _confirmDelete(
    ...
    if (confirmed == true && context.mounted) {
      await context.read<ProblemMasterProvider>().delete(row.id);
    }
  }

  void _handleExport(
    BuildContext context, {
    List<ProblemMasterModel>? rows,
  }) {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(
  ...
  Widget build(BuildContext context) {
    ...
    return Material(
      ...
      child: AppListingScreen<ProblemMasterModel>(
        title: 'Problem Master',
        subtitle: 'Problem categories for analysis',
        primaryActionLabel: '+ Add Problem',
        onPrimaryAction: () => ProblemMasterFormModal.show(context),
        showCheckboxes: true,
        showKpis: false,
        bulkRowId: (r) => r.id,
        ...
        onBulkExport: (rows) async => _handleExport(
          context,
          rows: rows.cast<ProblemMasterModel>().toList(),
        ),
        tabs: [
        ...
        columns: [
          TableColumn<ProblemMasterModel>(
            key: 'code',
            label: 'Code',
            width: 120,
            sortable: false,
        ...
        rowActions: [
          RowAction<ProblemMasterModel>(
            key: 'edit',
            label: 'Edit',
            icon: const Icon(LucideIcons.pencil),
            onTap: (row) => ProblemMasterFormModal.show(context, existing: row),
          ),
          RowAction<ProblemMasterModel>(
            key: 'toggle',
            label: 'Activate',
            ...
            onTap: (row) async {
              await context.read<ProblemMasterProvider>().toggleStatus(row.id);
            },
          ),
          RowAction<ProblemMasterModel>(
            key: 'delete',
