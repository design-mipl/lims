...
import '../state/sub_assembly_master_provider.dart';
import 'sub_assembly_master_form_modal.dart';

class SubAssemblyMasterScreen extends StatefulWidget {
  const SubAssemblyMasterScreen({super.key});

  @override
  State<SubAssemblyMasterScreen> createState() =>
      _SubAssemblyMasterScreenState();
}

class _SubAssemblyMasterScreenState extends State<SubAssemblyMasterScreen> {
  SubAssemblyMasterProvider? _provider;
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (!mounted) return;
      _provider = context.read<SubAssemblyMasterProvider>();
  ...
  Future<void> _confirmDelete(
    ...
    if (confirmed == true && context.mounted) {
      await context.read<SubAssemblyMasterProvider>().delete(row.id);
    }
  }

  void _handleExport(
    BuildContext context, {
    List<SubAssemblyMasterModel>? rows,
  }) {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(
  ...
  Widget build(BuildContext context) {
    final p = context.watch<SubAssemblyMasterProvider>();
    final filteredTotal = p.filteredItems.length;

    return Material(
      type: MaterialType.transparency,
      child: AppListingScreen<SubAssemblyMasterModel>(
        title: 'Sub Assembly Master',
        subtitle: 'Equipment sub-assemblies',
        primaryActionLabel: '+ Add Sub Assembly',
        onPrimaryAction: () => SubAssemblyMasterFormModal.show(context),
        showCheckboxes: true,
        showKpis: false,
        bulkRowId: (r) => r.id,
        ...
        onBulkExport: (rows) async => _handleExport(
          context,
          rows: rows.cast<SubAssemblyMasterModel>().toList(),
        ),
        tabs: [
        ...
        columns: [
          TableColumn<SubAssemblyMasterModel>(
            key: 'code',
            label: 'Code',
            width: 120,
            sortable: false,
        ...
        emptyMessage: 'No sub assemblies match your filters',
        onSearch: p.setSearchQuery,
        searchHint: 'Search by code or name…',
        rowActions: [
          RowAction<SubAssemblyMasterModel>(
            key: 'edit',
            label: 'Edit',
            icon: const Icon(LucideIcons.pencil),
            onTap: (row) =>
                SubAssemblyMasterFormModal.show(context, existing: row),
          ),
          RowAction<SubAssemblyMasterModel>(
            key: 'toggle',
            label: 'Activate',
            ...
            onTap: (row) async {
              await context
                  .read<SubAssemblyMasterProvider>()
                  .toggleStatus(row.id);
            },
          ),
          RowAction<SubAssemblyMasterModel>(
