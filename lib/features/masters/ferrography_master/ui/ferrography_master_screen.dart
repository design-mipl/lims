...
import 'ferrography_master_form_modal.dart';

class FerrographyMasterScreen extends StatefulWidget {
  const FerrographyMasterScreen({super.key});

  @override
  State<FerrographyMasterScreen> createState() =>
      _FerrographyMasterScreenState();
}

class _FerrographyMasterScreenState extends State<FerrographyMasterScreen> {
  FerrographyMasterProvider? _provider;
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (!mounted) return;
      _provider = context.read<FerrographyMasterProvider>();
  ...
  Future<void> _confirmDelete(
    ...
    FerrographyMasterModel row,
  ) async {
    final confirmed = await AppConfirmDialog.show(
      context: context,
      title: 'Delete ferrography',
      message: 'Delete "${row.code}" — ${row.name}?',
      confirmLabel: 'Delete',
      variant: AppConfirmDialogVariant.danger,
    );
    if (confirmed == true && context.mounted) {
      await context.read<FerrographyMasterProvider>().delete(row.id);
    }
  }

  void _handleExport(
    BuildContext context, {
    List<FerrographyMasterModel>? rows,
  }) {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(
  ...
  Widget build(BuildContext context) {
    ...
    return Material(
      ...
      child: AppListingScreen<FerrographyMasterModel>(
        title: 'Ferrography Master',
        subtitle: 'Ferrography parameters',
        primaryActionLabel: '+ Add Ferrography',
        onPrimaryAction: () => FerrographyMasterFormModal.show(context),
        showCheckboxes: true,
        showKpis: false,
        bulkRowId: (r) => r.id,
        ...
        onBulkExport: (rows) async => _handleExport(
          context,
          rows: rows.cast<FerrographyMasterModel>().toList(),
        ),
        tabs: [
        ...
        columns: [
          TableColumn<FerrographyMasterModel>(
            key: 'code',
            label: 'Code',
            width: 120,
            sortable: false,
        ...
        emptyMessage: 'No ferrography records match your filters',
        onSearch: p.setSearchQuery,
        searchHint: 'Search by code or name…',
        rowActions: [
          RowAction<FerrographyMasterModel>(
            key: 'edit',
            label: 'Edit',
            icon: const Icon(LucideIcons.pencil),
            onTap: (row) =>
                FerrographyMasterFormModal.show(context, existing: row),
          ),
          RowAction<FerrographyMasterModel>(
            key: 'toggle',
            label: 'Activate',
            ...
            onTap: (row) async {
              await context
                  .read<FerrographyMasterProvider>()
                  .toggleStatus(row.id);
            },
          ),
          RowAction<FerrographyMasterModel>(
