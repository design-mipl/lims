...
import '../data/lab_manager_verification_api.dart';

class LabManagerVerificationProvider extends BaseProvider {
  LabManagerVerificationProvider({LabManagerVerificationApi? api})
      : _api = api ?? sl<LabManagerVerificationApi>();

  final LabManagerVerificationApi _api;
  ...
  int get pendingVerificationCount =>
      items.where((e) => !labManagerListingVerificationComplete(e)).length;

  int get completedVerificationCount =>
      items.where((e) => labManagerListingVerificationComplete(e)).length;
  ...
  List<LabManagerListingRow> get filteredItems {
    Iterable<LabManagerListingRow> rows = items;
    if (_tabIndex == 0) {
      rows = rows.where((e) => !labManagerListingVerificationComplete(e));
    } else {
      rows = rows.where((e) => labManagerListingVerificationComplete(e));
    }
